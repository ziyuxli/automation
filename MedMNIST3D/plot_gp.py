...
SAVE_PATH = "passive_gaussian.png"
...
def plot_all(df):
    ...
    h_gaussian = plot_with_variance(
        ax,
        gaussian["labeled"].values,
        gaussian["val_loss_mean"].values,
        gaussian["val_loss_std"].values,
        "Gaussian Dropout",
        color_gaussian,
        marker="s",
    )
    ...
    plot_with_variance(
        ax,
        gaussian["labeled"].values,
        gaussian["val_auc_mean"].values,
        gaussian["val_auc_std"].values,
        "Gaussian Dropout",
        color_gaussian,
        marker="s",
    )
    ...
    fig.legend(
        handles=[h_passive, h_gaussian],
        labels=["Passive", "Gaussian Dropout"],
        loc="upper center",
    ...
    fig.suptitle("Passive vs Gaussian Dropout", fontsize=18, y=0.995)
...
def main():
    ...
    with open(GAUSSIAN_LOG, "r", encoding="utf-8") as f:
